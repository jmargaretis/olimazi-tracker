...
def render(folder: Path) -> Path:
    ...
    html = f'''<!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>Client Organizer — Sch. E — {escape(first_address)}</title>
...
<div class=hero><h1>Client Organizer</h1><p>{escape(first_address)} · {len(properties)} property · Tax year {escape(display(filing.get("tax_year")))}</p></div>
